...
unsigned int nStakeMinAge = 12 * 60 * 60; // 4 hours
...
const string strMessageMagic = "METRO Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (nBestHeight > 0 && nBestHeight < 101)
    {
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (nBestHeight < 20000)  // 100 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 1;
    }
    
    else if (nBestHeight < 40000) // 800 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 8;
	}

    else if (nBestHeight < 60000) // 400 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 4;
	}

    else if (nBestHeight < 80000) // 200 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 2;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "METRO", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The city's full of people who you just see around.";
        CTransaction txNew;
        txNew.nTime = 1545944736;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1545944736;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1289918 : 1289918;
        ...
        assert(block.hashMerkleRoot == uint256("0x805a23f4dfaaa33679ee6a1c67b0615e686ed019766a090b80e0c7fc79fdc6d5"));
...
unsigned char pchMessageStart[4] = { 0x9a, 0xd1, 0x28, 0xe9 };
